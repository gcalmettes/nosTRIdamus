...
class BaseRecommender:
    def __init__(self, model, matrix=None, items_info=None,
                 pos_to_item_mapping=None, df=None, name='Model'):
        self.name = name
        self.model = model
        if self.model is None:
            # Default model is Nearest Neighbors
            self.model = NearestNeighbors(
                metric='cosine', algorithm='brute', n_neighbors=5)
        self.matrix = matrix
        self.df = df
        if pos_to_item_mapping:
            self.items_info = items_info.loc[
                list(pos_to_item_mapping.values())]
        elif df is not None:
...
class ALSRecommender(BaseRecommender):
    ...
    def recommend(self, target, n=10, filterByField=False,
                  valueToMatch=False, months_range=[0, 12], options={}):
        ...
        df_order = df_order.loc[(df_order['month'] > months_range[0]) &
                                (df_order['month'] <= months_range[1])]
...
class KNNRecommender(BaseRecommender):
    ...
    def recommend(self, target, n=10, filterByField=False,
        ...
        (distances, indices) = self.model.kneighbors(
            df.loc[target].values.reshape(1, -1),
            n_neighbors=len(self.items_info)
        )
        df_distances = pd.concat([
            pd.Series(self.items_info_reset.loc[indices[0], 'race'].values,
                      name='race'),
    ...
    def getTransformedMatrix(self, target, options):
        ...
        if options['raceExperience'] == 2:
            ...
            # also on bike shops/poos/athletic centers, if they want to go
            # train there
            to_transform['wc_slots'] = {
                'dial_query': 5,
                'dial': 5,
                'increase': True
            }
            to_transform['n_bike_shops'] = {
                'dial_query': 3,
                'dial': 3,
                'increase': True
            }
            to_transform['n_pools'] = {
                'dial_query': 3,
                'dial': 3,
                'increase': True
            }
            to_transform['n_athletic_centers'] = {
                'dial_query': 3,
                'dial': 3,
                'increase': True
            }
        if options['raceExperience'] == 0:
            ...
            to_transform['distance_to_nearest_shoreline'] = {
                'dial_query': 10,
                'dial': 5,
                'increase': False
            }
            to_transform['n_restaurants'] = {
                'dial_query': 2,
                'dial': 1.5,
                'increase': True
            }
            to_transform['n_entertainment'] = {
                'dial_query': 3,
                'dial': 2,
                'increase': True
            }

        to_transform['bike_score'] = {
            'dial_query': options['raceDifficulty'],
            'dial': options['raceDifficulty'],
            'increase': True
        }
        to_transform['run_score'] = {
            'dial_query': options['raceDifficulty'],
            'dial': options['raceDifficulty'],
            'increase': True
        }
        to_transform['entrants_count_avg'] = {
            'dial_query': options['raceSize'],
            'dial': options['raceSize'],
            'increase': True
        }
        ...
        for col in to_transform.keys():
            data_transformed[col] = transform_col(
                data_transformed, col, to_transform[col]['dial'],
                increase=to_transform[col]['increase'])
            data_transformed.loc[target, col] = transform_query(
                to_transform[col]['dial_query'])
...
def transform_col(df, column, dial, increase=True):
    ...
    if increase:
        return dial * sigmo_transform(
            df[column].values, 0, 1, 0.5, factor)[:, np.newaxis]
    else:
        return dial * (1 - sigmo_transform(
            df[column].values, 0, 1, 0.5, factor))[:, np.newaxis]
